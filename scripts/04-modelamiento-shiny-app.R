# =============================================================================
# 04 — Modelamiento y evaluación
# Proyecto: Predicción de mortalidad en UCI (WiDS Datathon 2020)
# Autores: Yalbi Balderas, Jaime Soria, Belén Escola
# Descripción: Entrenamiento y evaluación de modelos de clasificación
#              (regresión logística, ElasticNet, Random Forest, LightGBM,
#              MARS, Naive Bayes) usando tidymodels.
# =============================================================================
...
UCI_noCat_rec <- recipe(hospital_death ~ ., data = UCI_noCat_training) %>%
  ...
  step_interact(
    terms = ~ heart_rate_apache:(heart_rate_cat_X..Inf.50. +
      heart_rate_cat_X.50.100.)
  ) %>% #heart_rate_cat_2 parece que no interactua
  step_interact(
    terms = ~ resprate_apache:(resprate_cat_X..Inf.10. + resprate_cat_X.10.35.)
  ) %>%
  ...
  step_interact(
    terms = ~ urineoutput_apache:urineoutput_cat_X..Inf.1.4e.03.
  ) %>%
...
#ElasticNet
UCI_noCat_lgnet_res <- UCI_noCat_wf %>%
  add_model(UCI_noCat_lgnet) %>%
  fit_resamples(
    resamples = UCI_noCat_folds,
    metrics = metric_set(
      roc_auc,
...
#Random forest
UCI_noCat_rf_res <- UCI_noCat_wf %>%
  add_model(UCI_noCat_rf) %>%
  fit_resamples(
    resamples = UCI_noCat_folds,
    metrics = metric_set(
      roc_auc,
...
#light gbm
UCI_noCat_lgbm_res <- UCI_noCat_wf %>%
  add_model(UCI_noCat_lgbm) %>%
  fit_resamples(
    resamples = UCI_noCat_folds,
    metrics = metric_set(
      roc_auc,
...
#Bayes
UCI_noCat_nbay_res <- UCI_noCat_wf %>%
  add_model(UCI_noCat_nbay) %>%
  fit_resamples(
    resamples = UCI_noCat_folds,
    metrics = metric_set(
      roc_auc,
...
collect_metrics(UCI_noCat_lgnet_res)
collect_metrics(UCI_noCat_rf_res)
collect_metrics(UCI_noCat_lgbm_res)
collect_metrics(UCI_noCat_mars_res)
collect_metrics(UCI_noCat_nbay_res)
...
#otros modelos
UCI_noCat_rf_final <- UCI_noCat_wf %>%
  add_model(UCI_noCat_rf) %>%
  last_fit(UCI_noCat_split)

UCI_noCat_lgbm_final <- UCI_noCat_wf %>%
  add_model(UCI_noCat_lgbm) %>%
  last_fit(UCI_noCat_split)
...
collect_metrics(UCI_noCat_rf_final)
collect_metrics(UCI_noCat_lgbm_final)

collect_predictions(UCI_noCat_lg_final) %>%
  conf_mat(hospital_death, .pred_class)

collect_predictions(UCI_noCat_rf_final) %>%
  conf_mat(hospital_death, .pred_class)

#ver el beta
UCI_noCat_lg_final %>%
  pull(.workflow) %>% #trae el workflow del modelo UCI_noCat_lg_final
...
UCI_noCat_lg_final %>%
  ...
  tidy() %>%
  filter(
    term != "hospital_admit_source_Observation", #
  ...
  geom_vline(xintercept = 0, color = "gray50", lty = 2, linewidth = 1) +
  geom_point() +
  geom_errorbar(
...
UCI_noCat_lg_final %>%
  ...
  tidy() %>%
  filter(
    term == "arf_apache_No " | #falla renal
...
set.seed(1234)
cv_data_demo <- vfold_cv(
  data_demo,
  v = 10,
  repeats = 1,
  strata = hospital_death
)
cv_data_demo

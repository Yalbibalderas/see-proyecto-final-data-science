# =============================================================================
# 02 — Preprocesamiento y análisis exploratorio
# Proyecto: Predicción de mortalidad en UCI (WiDS Datathon 2020)
# Autores: Yalbi Balderas, Jaime Soria, Belén Escola
# Descripción: Análisis exploratorio de datos (EDA), visualización de
#              distribuciones y evaluación de modelos preliminares (LightGBM, GAM).
# =============================================================================
...
library(mgcv)     # GAM (Generalized Additive Models)
...
# Muerte por edad
data_demo %>%
  count(age = 5 * (age %/% 5), hospital_death) %>%
  pivot_wider(names_from = hospital_death, values_from = n)
...
data_demo %>%
  count(age = 5 * (heart_rate_apache %/% 5), hospital_death) %>%
  pivot_wider(names_from = hospital_death, values_from = n)

data_demo %>%
  filter(!is.na(heart_rate_apache)) %>%
  mutate(
    heart_rate_apache = fct_lump(as.character(heart_rate_apache), prop = 0.03)
  ) %>%
...
#histograma de frecuencia cardiaca y mortalidad
ggplot(data_demo, aes(heart_rate_apache)) +
  ...
  labs(
    ...
    y = 'Porcentaje',
    title = 'Histograma de la cantidad de personas que
que fallecen según la frecuencia cardiaca en UCI'
  )

#histograma ponderado de frecuencia cardiaca y mortalidad
ggplot(data_demo, aes(heart_rate_apache)) +
  geom_histogram(aes(fill = hospital_death), binwidth = 10, position = 'fill') +
  labs(
    x = 'Frecuencia cardiaca',
...
data_demo %>%
  count(map_apache = 5 * (map_apache %/% 5), hospital_death) %>%
  pivot_wider(names_from = hospital_death, values_from = n)
...
#histograma de presión arterial media y mortalidad
ggplot(data_demo, aes(map_apache)) +
  geom_histogram(aes(fill = hospital_death), binwidth = 10) +
  labs(
    x = 'Presión arterial media',
    y = 'Porcentaje',
    title = 'Histograma de la cantidad de personas que
que fallecen según la presión arterial media en UCI'
  )

#histograma ponderado de presión arterial media y mortalidad
ggplot(data_demo, aes(map_apache)) +
  geom_histogram(aes(fill = hospital_death), binwidth = 10, position = 'fill') +
  labs(
    x = 'Presión arterial media',
...
ggplot(
  data_demo,
  aes(temp_apache_hipo, map_apache_hipo, color = hospital_death)
) +
  geom_point()
...
data_UCI %>% summarise_all(~sum(is.na(.)))
...
# temp_apache
prop.table(
  table(is.na(data_UCI$temp_apache), data_UCI$hospital_death),
  margin = 1
)
...
# bun_apache
prop.table(
  table(is.na(data_UCI$bun_apache), data_UCI$hospital_death),
  margin = 1
)
...
# glucose_apache
prop.table(
  table(is.na(data_UCI$glucose_apache), data_UCI$hospital_death),
  margin = 1
)
...
# hematocrit_apache
prop.table(
  table(is.na(data_UCI$hematocrit_apache), data_UCI$hospital_death),
  margin = 1
)
...
# wbc_apache
prop.table(
  table(is.na(data_UCI$wbc_apache), data_UCI$hospital_death),
  margin = 1
)
...
# sodium_apache
prop.table(
  table(is.na(data_UCI$sodium_apache), data_UCI$hospital_death),
  margin = 1
)
...
data_UCI_clean %>% summarise_all(~sum(is.na(.)))
dim(data_UCI_clean)
#58644    41
...
data_UCI_gas %>% summarise_all(~sum(is.na(.)))

prop.table(
  table(is.na(data_UCI_gas$fio2_apache), data_UCI_gas$hospital_death),
  margin = 1
)
...
data_UCI_gas_clean %>% summarise_all(~sum(is.na(.)))
...
data_UCI_gas_cat_clean %>% summarise_all(~sum(is.na(.)))
